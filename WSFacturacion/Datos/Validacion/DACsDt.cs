...
namespace Telectronica.Validacion
{
    public class DACsDt
    {
        /// ***********************************************************************************************
        /// <summary>
        /// lista de anomalias de tipo DAC
        /// </summary>
        ...
        public static DataSet getAnomalias(Conexion oConn, int codAnomalia, ParteValidacion parte, bool puedeVerValInvisible)
        {
            ...
            dsAnomalias.DataSetName = "DACs_AnomaliasDS";
            try
            {
                ...
                oCmd.CommandText = "Validacion.usp_Dacs_getAnomalias";
        ...
        public static void setAnomaliasValidadas(Conexion oConn, int codAnomalia, int estacion, int parte, DateTime jornada, AnomaliaValidacionL anomalias, string validador)
        {
            try
            {
                ...
                SqlMetaData[] tvp_definition = { new SqlMetaData("Validada", SqlDbType.Char, 1),
                                             ...
                                             new SqlMetaData("EjeAdicionalDetectado", SqlDbType.Int),
                ...
                foreach (AnomaliaValidacion item in anomalias)
                {
                    ...
                    if (item.RecNumev != 0) rec.SetInt32(38, item.RecNumev); else rec.SetDBNull(38);
                    if (item.FechaRecarga != null) rec.SetDateTime(39, (DateTime)item.FechaRecarga); else rec.SetDBNull(39);

                    if (item.EjeAdicionalTabulado != 0) rec.SetInt32(40, Convert.ToInt32(item.EjeAdicionalTabulado)); else rec.SetDBNull(40);
                    if (item.EjeAdicionalConsolidado != 0) rec.SetInt32(41, Convert.ToInt32(item.EjeAdicionalConsolidado)); else rec.SetDBNull(41);
                    if (item.EjeAdicionalDetectado != 0) rec.SetInt32(42, Convert.ToInt32(item.EjeAdicionalDetectado)); else rec.SetDBNull(42);

                    if (item.EjeSuspensoConsolidado != null)
                        rec.SetByte(43, item.EjeSuspensoConsolidado);
                    else
                        rec.SetDBNull(43);
                ...
                oCmd.CommandType = System.Data.CommandType.StoredProcedure;
                oCmd.CommandText = "Validacion.usp_Dacs_SetAnomaliasValidadas";
                ...
                oCmd.Parameters["@anomalias"].TypeName = "listaDACs";
